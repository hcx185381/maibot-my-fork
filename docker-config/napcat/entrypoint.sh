...
export DISPLAY=:1
cd /app/napcat
# 添加 --disable-gpu 和 --disable-software-rasterizer 参数以修复 Windows Docker 环境下的 GPU 问题
if [ -n "${ACCOUNT}" ]; then
    gosu napcat /opt/QQ/qq --no-sandbox --disable-gpu --disable-software-rasterizer -q $ACCOUNT
else
